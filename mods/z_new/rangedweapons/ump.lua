minetest.register_craft({
	...
	minetest.register_craftitem("rangedweapons:ump", {
	stack_max= 1,
...
minetest.register_globalstep(function(dtime, player)
	...
	local wielded_item = player:get_wielded_item():get_name()
		if wielded_item == "rangedweapons:ump" then
...
minetest.sound_play("rangedweapons_empty", {object=player})
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
			local obj = minetest.add_entity(pos, "rangedweapons:umpshot")
			if obj then
...
rangedweapons_umpshot.on_step = function(self, dtime, node, pos)
	...
	if self.timer > 0.12 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "rangedweapons:umpshot" and obj:get_luaentity().name ~= "__builtin:item" then
					if math.random(1, 100) <= 8 then
					local damage = math.random(9,18)
...
minetest.register_entity("rangedweapons:umpshot", rangedweapons_umpshot )