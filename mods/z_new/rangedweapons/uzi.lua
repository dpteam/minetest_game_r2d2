minetest.register_craft({
	...
	minetest.register_craftitem("rangedweapons:uzi", {
	stack_max= 1,
...
minetest.register_globalstep(function(dtime, player)
	...
	local wielded_item = player:get_wielded_item():get_name()
		if wielded_item == "rangedweapons:uzi" then
...
minetest.sound_play("rangedweapons_empty", {object=player})
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
			local obj = minetest.add_entity(pos, "rangedweapons:uzishot")
			if obj then
				minetest.sound_play("rangedweapons_machine_pistol", {object=obj})
				obj:setvelocity({x=dir.x * 25, y=dir.y * 25, z=dir.z * 25})
				obj:setacceleration({x=dir.x * math.random(-3.1,3.1), y=math.random(-3.1,3.1), z=dir.z * math.random(-3.1,3.1)})
...
rangedweapons_uzishot.on_step = function(self, dtime, node, pos)
	...
	if self.timer > 0.125 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "rangedweapons:uzishot" and obj:get_luaentity().name ~= "__builtin:item" then
...
minetest.register_entity("rangedweapons:uzishot", rangedweapons_uzishot )